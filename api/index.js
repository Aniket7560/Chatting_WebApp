...
app.use(cors({
    credentials: true,
    origin : ["https://chatting-web-app-api.vercel.app"],
...
app.get('/messages/:userId', async (req, res) => {
    try {
        ...
        const decryptedMessages = messages.map(message => ({
            ...message._doc,
            text: decrypt(message.text, message.iv) // Decrypt the message text
        }));
    ...
    connection.on('message', async (message) => {
        ...
        if (to && (text || file)) {
            ...
            const messageDoc = await Message.create({
                ...
                iv: encryptedMessage.iv, // Store the IV for decryption
                file: filename,
            });
            console.log(to);
            console.log(text);
